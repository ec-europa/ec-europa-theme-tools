...
//      var blockWidth = $parent.width() + 'px';
//      $e.css({
//        'width' : blockWidth
//      });
//    },
...
(function ($) {
  Drupal.behaviors.inpage_navigation = {
    attach: function (context) {
      ...
      var selector = '.inpage-nav',
        topOffset = GetOffsetTop('.inpage-nav__wrapper'),
        screenWidth = $(window).width();

      // When we resize the window we should recalculate the top.
      $(window).resize(function () {
        topOffset = GetOffsetTop('.inpage-nav__wrapper');
        screenWidth = $(window).width();
      });

      $('.inpage-nav__navbar-wrapper').hide();
      $(selector).hide();
      ...
      $(window).scroll(function () {
        ...
        if (screenWidth < 991) {
          ...
          if ($(window).scrollTop() > topOffset) {
            console.log('mobile');
            $(selector).hide();
            $('.inpage-nav__navbar-wrapper').show();
          }
          else {
            $(selector).show();
            $('.inpage-nav__navbar-wrapper').hide();
